...
class Meme: NSObject, NSCoding {
    ...
    required convenience init?(coder aDecoder: NSCoder) {
        ...
        guard let name = aDecoder.decodeObject(forKey: PropertyKey.name) as? String else {
            os_log("Unable to decode the name for a Meme object.", log: OSLog.default, type: .debug)
            return nil
        }
        
        // Because photo is an optional property of Meme, just use conditional cast.
